...
"""
rf_pose_tuning.py
Test multiple Random Forest configurations (n_estimators, max_depth) on extracted yoga pose features.
"""
...
NUM_WORKERS = 12
CACHE_FILE = "pose_features_rf.pkl"
...
def main():
    ...
    df.to_csv("rf_precision_comparison.csv", index=False)
    print("\nSaved comparison -> rf_precision_comparison.csv")
    print(df)

    plt.figure(figsize=(10,6))
    sns.barplot(x="n_estimators", y="precision", hue="max_depth", data=df)
    plt.title("Random Forest Precision vs n_estimators (grouped by max_depth)")
    plt.tight_layout()
    plt.savefig("rf_precision_barplot.png")
    plt.close()
    print("Saved precision comparison plot -> rf_precision_barplot.png")
