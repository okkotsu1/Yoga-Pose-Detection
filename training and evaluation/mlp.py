...
"""
mlp_pose_tuning.py
Test multiple MLP configurations (hidden layers, activation, solver)
on extracted yoga pose features and evaluate precision.
"""
...
NUM_WORKERS = 12
CACHE_FILE = "pose_features_mlp.pkl"
...
def main():
    ...
    for hls in hidden_layer_sizes:
        for act in activations:
            for sol in solvers:
                print(f"\nConfig -> Layers={hls}, Activation={act}, Solver={sol}")
                mlp = MLPClassifier(
    ...
    df.to_csv("mlp_precision_comparison.csv", index=False)
    print("\nSaved comparison -> mlp_precision_comparison.csv")
    print(df)

    plt.figure(figsize=(12, 6))
    sns.barplot(x="Activation", y="Precision", hue="Solver", data=df)
    plt.title("MLP Precision Comparison (Different Activations & Solvers)")
    plt.tight_layout()
    plt.savefig("mlp_precision_barplot.png")
    plt.close()
    print("Saved precision comparison plot -> mlp_precision_barplot.png")
