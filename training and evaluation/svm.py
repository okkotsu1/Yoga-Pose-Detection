...
"""
svm_pose_tuning.py
Test multiple SVM configurations (kernel, loss) on extracted yoga pose features.
"""
...
NUM_WORKERS = 12
CACHE_FILE = "pose_features_svm.pkl"
...
def main():
    ...
    # Loss functions to test (LinearSVC)
    losses = ['hinge', 'squared_hinge']

    results = []

    # ---- SVC kernel test ----
    print("\n--- Testing different kernels (SVC) ---")
    for kernel in kernels:
        ...
        results.append({
            "model_type": "SVC",
            "kernel_or_loss": kernel,
            "accuracy": acc,
            "precision": prec
        })

    # ---- LinearSVC loss test ----
    print("\n--- Testing different losses (LinearSVC) ---")
    for loss in losses:
        print(f"\nLoss = {loss}")
        lsvc = LinearSVC(loss=loss, C=1.0, random_state=42, max_iter=5000)
    ...
    df.to_csv("svm_precision_comparison.csv", index=False)
    print("\nSaved comparison -> svm_precision_comparison.csv")
    print(df)

    plt.figure(figsize=(10,6))
    sns.barplot(x="kernel_or_loss", y="precision", hue="model_type", data=df)
    plt.title("SVM Precision Comparison (Kernel & Loss)")
    plt.tight_layout()
    plt.savefig("svm_precision_barplot.png")
    plt.close()
    print("Saved precision comparison plot -> svm_precision_barplot.png")
